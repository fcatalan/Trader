...
def plot_rolling_mean(df, ax):
    rm_BTC = df['Close'].rolling(window=20).mean() #pd.rolling_mean(df['Close'], window=WINDOW)
...
def plot_bollinger_bands(rm_BTC, df, ax):
    rstd_BTC = df['Close'].rolling(window=20).std() #pd.rolling_std(df['Close'], window=WINDOW)
